...
from f1_replay.api.serializers import serialize_telemetry, to_json_safe  # noqa: E402
from f1_replay.log import logger  # noqa: E402
from f1_replay.managers import DataLoader  # noqa: E402
from f1_replay.wrappers import RaceWeekend, Session, create_session  # noqa: E402


def _get_scheduled_session_info(
    data_loader: DataLoader, year: int, round_num: int, session_type: str
) -> Optional[dict]:
    """
...
def create_app(
    data_loader: DataLoader, current_session: Optional[Session] = None, force_update: bool = False
) -> Flask:
    """
        ...
        try:
            ...
            if cache_key in app.config["WEEKEND_CACHE"]:
                ...
                weekend_data = data_loader.load_weekend(
                    year, round_num, event, force_reprocess=app.config.get("FORCE_UPDATE", False)
                )
    ...
    def get_session(year: int, round_num: int, session_type: str):
        ...
        try:
            ...
            if (
                app.config["CURRENT_SESSION"] is not None
                and app.config["CURRENT_SESSION"].year == year
                and app.config["CURRENT_SESSION"].round_number == round_num
                and app.config["CURRENT_SESSION"].session_type == session_type
            ):
            ...
            # Check in-memory cache
                ...
                logger.debug(
                    f"  drivers: {session.drivers}, telemetry: {len(session.telemetry)} drivers"
                )
                logger.info(
                    f"✓ Session loaded via API: {session.event_name} {session_type} ({len(session.drivers)} drivers)"
                )
            ...
            return fast_jsonify(
                {
                    ...
                    "telemetry": serialize_telemetry(
                        session_data.telemetry, fields=telemetry_fields
                    ),
