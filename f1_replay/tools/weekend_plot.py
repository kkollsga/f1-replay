...
def plot_weekend(
    ...
    if color_mode == "sectors" and circuit.track.marshal_sectors:
        ...
        lc = LineCollection(
            segments, cmap="plasma", norm=norm, linewidth=track_width, capstyle="round", zorder=4
        )
        lc.set_array(speed)
        ...
        throttle = (
            circuit.track.throttle
            if circuit.track.throttle is not None
            else np.ones_like(brake) * 100
        )
        ...
        brake_norm = (
            np.clip(brake / brake_max, 0, 1) if brake_max <= 1 else np.clip(brake / 100.0, 0, 1)
        )
        ...
        lc_brake = LineCollection(
            segments, colors=brake_colors, linewidth=track_width, capstyle="round", zorder=3
        )
        ...
        lc_throttle = LineCollection(
            segments, colors=throttle_colors, linewidth=track_width, capstyle="round", zorder=4
        )
        ...
        cmap = LinearSegmentedColormap.from_list(
            "elevation", [(pos, color) for pos, color in colors]
        )
        ...
        if contour_settings["enabled"]:
            ...
            contour_levels = np.arange(
                z_min_rounded, z_max_rounded + contour_interval, contour_interval
            )
            ...
            if minor_lines:
                lc_minor = LineCollection(
                    minor_lines,
                    colors=contour_settings["color"],
                    linewidth=contour_settings["linewidth"],
                    alpha=contour_settings["alpha"],
                    zorder=6,
                )
            ...
            if major_lines:
                lc_major = LineCollection(
                    major_lines,
                    colors=contour_settings["color"],
                    linewidth=contour_settings.get(
                        "linewidth_major", contour_settings["linewidth"]
                    ),
                    alpha=contour_settings.get("alpha_major", contour_settings["alpha"]),
                    zorder=7,
                )
    ...
    if circuit.direction_arrow is not None:
        ...
        ax.plot(
            [arrow_end_x, arrow_end_x + left_dx * head_size],
        ...
        ax.plot(
            [arrow_end_x, arrow_end_x + right_dx * head_size],
    ...
    if save_path:
        fig.savefig(save_path, dpi=dpi, facecolor=BG_COLOR, edgecolor="none", bbox_inches="tight")
