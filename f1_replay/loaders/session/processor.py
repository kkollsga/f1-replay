...
class SessionProcessor:
    ...
    def build_session(self, year: int, round_num: int,
        ...
        logger.info(f"  ✓ Events built")

        # STEP 5: Build Telemetry LAST ★
        logger.info(f"  → Building telemetry from pos_data + car_data...")
        ...
        if not telemetry:
            logger.warning(f"  ⚠ No telemetry data available")
            return None
        ...
        if self.weekend_track is not None and self.weekend_track.x is not None:
            ...
            logger.info(f"  ✓ Track distance added from weekend track")
        else:
            # No track geometry available - add placeholder columns for status update
            logger.warning(f"  ⚠ No weekend track geometry, adding placeholder distance columns")
        ...
        logger.info(f"  ✓ Session complete: {len(metadata.drivers)} drivers, {len(telemetry)} with telemetry")
