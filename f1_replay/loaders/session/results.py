                ...
                try:
                    ...
                    if lap_time_seconds < current_fastest_time:
                        ...
                        if (
                            ...
                            and not pd.isna(lap_start)
                            and lap_time_td is not None
                            and not pd.isna(lap_time_td)
                        ):
                            try:
                                # Convert to seconds
            ...
            try:
                ...
                if standings:
                    ...
                    position_history.append(
                        PositionSnapshot(time=session_end_time, standings=standings)
                    )
...
def build_results(
    ...
    if telemetry:
        ...
        if first_driver:
            ...
            logger.info(
                f"  -> Building results with telemetry: {len(telemetry)} drivers, session_time={'yes' if has_session_time else 'no'}"
            )
    ...
    if fastest_laps or position_history:
        logger.info(
            f"  -> Results: {len(fastest_laps)} fastest laps, {len(position_history)} position snapshots"
        )

    return ResultsData(fastest_laps=fastest_laps, position_history=position_history)
